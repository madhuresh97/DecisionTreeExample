...
def findInformationGain(data, target, column, entropyParent):
    ...
    Information Gain = Entropy under current node - Weighted average of all decision paths from current node
    '''
...
def makeDecisionTree(data, target, parent=-1, path=''):
    ...
    for path in paths:
        ...
        modData = [x for x in data if splitColumn in x and x[splitColumn] == path]
        for y in modData:
            if splitColumn in y:
                del y[splitColumn]
